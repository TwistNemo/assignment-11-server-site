...
export default function createWebsocketServer({ clientName, server, maxPayloadBytes, ...workerOptions }) {
    ...
    worker.onResponse((response) => {
        switch (response.kind) {
            ...
            case 'module-error': {
                const resolvers = moduleResolvers.get(response.moduleID);
                if (resolvers) {
                    resolvers.reject(new Error(`Error building ${response.moduleID}: ${response.message}`));
                }
                break;
            }
        }
