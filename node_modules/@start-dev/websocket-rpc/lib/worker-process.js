...
import { RequestKind, ResponseKind } from './types';
            ...
            switch (request.kind) {
                ...
                case RequestKind.MethodCall:
                    ...
                    Promise.resolve(null)
                        .then(async () => {
                        return require(request.moduleID)[request.exportName](...request.args);
                    })
                        .then((value) => {
                        send({
                ...
                case RequestKind.Subscribe:
                    ...
                    Promise.resolve(null)
                        ...
                        unsubscribe.push(observable.subscribe((value) => {
                            const etag = getObjectEtag(value);
                            send({
                                kind: ResponseKind.SubscriptionUpdate,
                                id: request.id,
...
process.on('message', (message) => {
    switch (message.kind) {
        case 'open': {
            ...
            connections.set(connectionID, createConnection((response) => {
        ...
        case 'request': {
            ...
            if (connection)
                connection.onRequest(message.request);
            break;
        }
        case 'close': {
            const connectionID = message.connectionID;
        ...
        case 'get-module': {
            ...
            Promise.resolve(null)
                ...
                for (const methodName of Object.keys(pkg)) {
                    const declaration = methodName === 'default'
                        ? `export default`
                        : `export const ${methodName} =`;
                    if (typeof pkg[methodName] === 'function') {
                        output.push(`${declaration} ${clientName}.asyncMethod(__filename__, ${JSON.stringify(methodName)});`);
                    }
                    else if (pkg[methodName] &&
                        ...
                        output.push(`${declaration} ${clientName}.observableState(__filename__, ${JSON.stringify(methodName)}, ${JSON.stringify(initialEtag)}, ${JSON.stringify(initialValue)});`);
                    }
                }
                return output.join('\n');
            })
