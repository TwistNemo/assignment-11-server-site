...
function createWorker(options) {
  ...
  const worker = child_process.fork(
    workerPath,
    (_a = options.args) !== null && _a !== void 0 ? _a : process.argv.slice(2),
    {
...
function createWebsocketServer({
  ...
  wss.on('connection', (ws) => {
    ...
    console.log(source.cyan(`connection open (active count: ${++count})`));
    let closed = false;
    const onClose = () => {
      if (closed) return;
      closed = true;
      console.log(source.cyan(`connection close (active count: ${--count})`));
