...
var rollupPluginNodePolyfills = require('rollup-plugin-node-polyfills');
var fs = require('fs');
var vm2 = require('vm2');
...
var rollupPluginCommonjs__default = /*#__PURE__*/ _interopDefaultLegacy(
  rollupPluginCommonjs,
);
var path__default = /*#__PURE__*/ _interopDefaultLegacy(path);
var rollupPluginNodeResolve__default = /*#__PURE__*/ _interopDefaultLegacy(
  rollupPluginNodeResolve,
);
var rollupPluginNodePolyfills__default = /*#__PURE__*/ _interopDefaultLegacy(
...
var pm = _interopDefault$1(picomatch$1);
...
function getMatcherString(id, resolutionBase) {
  ...
  const basePath = path__default['default']
    .resolve(resolutionBase || '')
  ...
  return path__default['default'].posix.join(basePath, id);
}
const createFilter = function createFilter(include, exclude, options) {
  ...
  return function result(id) {
    ...
    const pathId = id.split(path__default['default'].sep).join('/');
...
var dataToEsm_1 = dataToEsm;

function json(options) {
  if (options === void 0) options = {};

  var filter = createFilter_1(options.include, options.exclude);
...
function isNodeFetch(id) {
  return (
    id === 'node-fetch' ||
    id === 'whatwg-fetch' ||
    id.includes(path__default['default'].join('node_modules', 'node-fetch')) || // note: sometimes Snowpack has found the entry file already
    id.includes(path__default['default'].join('node_modules', 'whatwg-fetch'))
  );
}
function rollupPluginNativeFetch() {
  return {
...
let Q;
const B = 1 === new Uint8Array(new Uint16Array([1]).buffer)[0];
function parse$1(g, I = '@') {
  if (!Q) throw new Error('Not initialized');
  const D = g.length + 1,
    N =
      (Q.__heap_base.value || Q.__heap_base) +
      4 * D -
      Q.memory.buffer.byteLength;
  N > 0 && Q.memory.grow(Math.ceil(N / 65536));
  const k = Q.sa(D);
  if (
    ((B ? C : E)(g, new Uint16Array(Q.memory.buffer, k, D)),
    !Q.parseCJS(k, g.length, 0, 0, 0))
  )
    throw Object.assign(
      new Error(
        `Parse error ${I}${Q.e()}:${g.slice(0, Q.e()).split('\n').length}:${
          Q.e() - g.lastIndexOf('\n', Q.e() - 1)
        }`,
      ),
      {idx: Q.e()},
  ...
  let w = new Set(),
    J = new Set(),
    H = new Set();
  for (; Q.rre(); ) J.add(g.slice(Q.res(), Q.ree()));
  for (; Q.ru(); ) H.add(g.slice(Q.us(), Q.ue()));
  for (; Q.re(); ) {
    let B = g.slice(Q.es(), Q.ee());
    A.has(B) || H.has(B) || w.add(B);
  }
    ...
    (g = (async () => {
      const A = await WebAssembly.compile(
        ...
        'function' == typeof atob
          ? Uint8Array.from(atob(B), (A) => A.charCodeAt(0))
          : Buffer.from(B, 'base64')),
      );
      var B;
      const {exports: E} = await WebAssembly.instantiate(A);
      Q = E;
    })())
  );
}
 ...
 */
function cjsAutoDetectExports(filename, visited = new Set()) {
  ...
  const fileContents = fs__default['default'].readFileSync(filename, 'utf-8');
  try {
    // Attempt 1 - CJS: Run cjs-module-lexer to statically analyze exports.
    const parsed = parse$1(fileContents);
    ...
    const resolvedReexports = reexports.flatMap((e) => {
      var _a;
      return (_a = cjsAutoDetectExports(
        require.resolve(e, {
          paths: [path__default['default'].dirname(filename)],
        }),
        visited,
      )) !== null && _a !== void 0
        ? _a
    ...
    if (isMainEntrypoint && exports.length === 0 && reexports.length === 0) {
      ...
      const vm = new vm2.VM({wasm: false, fixAsync: false});
      exports = Object.keys(
        vm.run(
...
function rollupPluginWrapCommmonJsEntrypoints() {
    ...
    async buildStart(inputOptions) {
      ...
      for (const [key, val] of Object.entries(input)) {
        const normalizedFileLoc = val
          .split(path__default['default'].win32.sep)
...
async function install(entrypoints, destLoc) {
  const inputOptions = {
    input: entrypoints,
    external: (id) => {
      if (path__default['default'].isAbsolute(id)) return false;
    ...
    plugins: [
      ...
      rollupPluginCommonjs__default['default']({
        extensions: ['.js', '.cjs'],
        // esmExternals: externalPackageEsm,
        requireReturnsDefault: 'auto',
      }),
      rollupPluginNodeResolve__default['default']({
      ...
      rollupPluginNodePolyfills__default['default'](),
  ...
  const outputOptions = {
    dir: path__default['default'].join(destLoc, ``),
    format: 'esm',
    sourcemap: true,
    exports: 'named',
