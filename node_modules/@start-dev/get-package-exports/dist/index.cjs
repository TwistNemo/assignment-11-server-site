...
async function getPackageExports(packageDirectory, options) {
  return new Promise((resolve, reject) => {
    ...
    function onExports(resolvedPackageDirectory, parts, exports, cb) {
      const pkgFileName = path.join(resolvedPackageDirectory, 'package.json');
      for (const [exportName, $exportPathArray] of Object.entries(exports)) {
        ...
        for (const exportPath of exportPathArray) {
          ...
          onExport({
            ...
            resolvedPath: path.join(resolvedPackageDirectory, exportPathString),
          });
          break;
        }
      }
    ...
    function testDirectory(dir, parts) {
      if (!options.overrideExports) {
        task((cb) => {
          fs.readdir(dir, (err, directoryNames) => {
            if (err) {
              cb();
              return;
            }
            for (const directoryName of directoryNames) {
              if (
                ...
                testDirectory(path.join(dir, directoryName), [
                  ...parts,
                  directoryName,
                ]);
              }
      ...
      task((cb) => {
        fs.readFile(pkgFileName, 'utf8', (err, result) => {
          ...
          const exports =
            (_a = options.overrideExports) !== null && _a !== void 0
              ? _a
              : pkg.exports;
    ...
    task((cb) => {
      fs.realpath(packageDirectory, (err, resolvedPath) => {
        if (err) {
          cb(err);
          return;
        }
...
module.exports = getPackageExports;
