...
function parse(E, g = '@') {
  ...
  const I = E.length + 1,
    D =
      (B.__heap_base.value || B.__heap_base) +
      4 * I -
      B.memory.buffer.byteLength;
  D > 0 && B.memory.grow(Math.ceil(D / 65536));
  const w = B.sa(I - 1);
  if (((A ? C : Q)(E, new Uint16Array(B.memory.buffer, w, I)), !B.parse()))
    throw Object.assign(
      new Error(
        `Parse error ${g}:${E.slice(0, B.e()).split('\n').length}:${
          B.e() - E.lastIndexOf('\n', B.e() - 1)
        }`,
      ),
      {idx: B.e()},
  ...
  for (; B.re(); ) k.push(E.slice(B.es(), B.ee()));
  return [L, k, !!B.f()];
}
function Q(A, Q) {
  const C = A.length;
...
async function rewriteImports({name, source}, resolve) {
  await init;
